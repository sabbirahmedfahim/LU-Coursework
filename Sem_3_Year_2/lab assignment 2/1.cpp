/*1. Find the difference between the maximum and minimum values of an element-linked list using the function.*/
...
int diff(Node * head) // the function will return an integer value
{
    if(head == NULL) return 0; // if the head if null, return zero
    int mx = head->data, mn = head->data; // make head pointers value min, max at first
    while (head != NULL) // iterate
    {
        mx = max(mx, head->data); // update if the condition as matched
        mn = min(mn, head->data); // update if the condition as matched
        head = head->next;
    }
    return (mx - mn); // return the answer
}
